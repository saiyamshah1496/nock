...
- After you’re on a recent CLI that emits catalogue sections, re‑run your scheduled `sync-estate --push-url ...` (or trigger it once manually) so the hosted snapshot includes `columns[]`/`constraints[]`/`indexes[]`.
...
- In public workflows, prefer `@nockhq/cli@latest` in `npx` commands.
