...
- Create/verify a least‑privilege read‑only role with catalogue grants. See [`docs/guides/grants-sync-estate.md`](./grants-sync-estate.md). Phase‑1 catalogue reads `pg_attribute`, `pg_constraint`, `pg_index` in addition to sizes.
- Prefer a read‑replica; store its DSN in a repo secret `PG_ESTATE_URL`.
- Use a current `@nockhq/cli@latest` (older CLIs won’t emit `columns[]`/`constraints[]`/`indexes[]`).
