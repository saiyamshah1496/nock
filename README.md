...
| R008 — ALTER TYPE non-binary-coercible | Implemented (catalogue-aware) | Yellow unknown; red when clearly rewriting (USING). Widen to text/varchar passes. When fresh catalogue is present, known-safe widens relative to `columns[].type_name` (e.g., integer→bigint, float4→float8, varchar(n)→varchar(m≥n)/text) are softened (no false rewrite alarm). Omitted catalogue → today’s R008 only (fail‑closed). |
